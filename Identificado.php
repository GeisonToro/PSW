...
for(i=0; i<jsonPHP.length; i++){
	var marcadorPHP= L.marker([jsonPHP[i][17],jsonPHP[i][18]]).addTo(map);
...
/*
function SendLatLong(lat,lng) {
    // Creating the XMLHttpRequest object
    var request = new XMLHttpRequest();
	var url = `subir_reporte.php?lat=${lat}&lng=${lng}`
	console.log(url)
...
function SendLatLong(lat,lng) {
    $.ajax({
                data:  {"lat" : lat, "lng" : lng}, //datos que se envian a traves de ajax
				
                url:   'subir_reporte.php', //archivo que recibe la peticion
...
function onMapClick(e) {
	...
	popup
		.setLatLng(e.latlng)
		//.setContent(lat+"</br>"+ lng + "</br>"+ reporte)
		
		.setContent("<center>Clic en REPORTAR para generar un reporte en esta ubicacion: </center>"+ reporte)
		.openOn(map);
